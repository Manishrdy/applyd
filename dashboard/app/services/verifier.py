...
async def _drain(
    ...
    if not jobs:
        return {"checked": 0}
    results = {"checked": 0, "active": 0, "expired": 0, "unknown": 0, "error": 0}
    async with httpx.AsyncClient(
        ...
        await asyncio.gather(*[_one(j, u, a) for (j, u, a) in jobs])
    return results
...
async def drain_suspected(batch_size: int | None = None) -> dict:
    ...
    return await _drain(jobs, trigger="http_check")
...
async def drain_manifest_drops(batch_size: int = 200) -> dict:
    ...
    return await _drain(jobs, trigger="manifest_drop")
